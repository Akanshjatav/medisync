...
public class HeadOfficeController {
    ...
    public StoreResponse updateBranch(
        ...
        ctx.requireRole("ADMIN");
        return toStoreResponse(storeService.updateBranch(id, request));
    }
    ...
    // ✅ NEW: Unallocated MANAGERS
    @GetMapping("/users/unallocated/managers")
    public List<UserResponse> getUnallocatedManagers() {
        ctx.requireRole("ADMIN");
        return usersService.getUnallocatedManagers()
                .stream()
                .map(this::toUserResponse)
                .collect(Collectors.toList());
    }

    // ✅ NEW: Unallocated PHARMACISTS
    @GetMapping("/users/unallocated/pharmacists")
    public List<UserResponse> getUnallocatedPharmacists() {
        ctx.requireRole("ADMIN");
        return usersService.getUnallocatedPharmacists()
                .stream()
                .map(this::toUserResponse)
                .collect(Collectors.toList());
    }
