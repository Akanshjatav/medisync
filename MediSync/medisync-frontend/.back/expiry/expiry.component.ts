...
import {
  ...
  clearSidebarBodyClass,
  closeSidebarState,
  confirmLogout,
  toggleSidebarState,
  getBranchFromStorage,
  checkIsBrowser
} from '../../src/app/shared/utils/pharmacist-ui.util';
import { PharmacistService } from '../../src/app/core/services/pharmacist.service';
import { ExpiryItem, ExpirySeverity } from '../../src/app/core/models/pharmacy.models';
...
export class ExpiryComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // Use stored branch name if your login flow sets it; else fallback placeholder
    this.branchName = this.isBrowser ? (getBranchFromStorage(this.isBrowser) ?? 'Current Branch') : 'Current Branch';
    this.fetchExpiring();
  }

  ngOnDestroy(): void {
    clearSidebarBodyClass(this.isBrowser, this.document);
  }

  // Sidebar
  toggleSidebar(): void {
    this.sidebarOpen = toggleSidebarState(this.isBrowser, this.document, this.sidebarOpen);
  }
  closeSidebar(): void {
    this.sidebarOpen = closeSidebarState(this.isBrowser, this.document);
  }
